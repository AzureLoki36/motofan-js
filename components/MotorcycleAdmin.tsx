...
export default function MotorcycleAdmin({ motoType, motorcycles, onSaved }: Props) {
  ...
  const openAdd = () => { setEditId(null); setForm(emptyForm(motoType)); setView("form"); };

  const openEdit = (m: Motorcycle) => {
    setEditId(m.id);
    setForm({ type: m.type, brand: m.brand, model: m.model, year: m.year, mileage: m.mileage, displacement: m.displacement, power: m.power, price: m.price, description: m.description, status: m.status, category: m.category, images: [...m.images] });
    setView("form");
  };
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      if (res.ok) { onSaved(); setView("list"); setEditId(null); setForm(emptyForm(motoType)); }
  ...
  if (!open) {
    return (
      <button className="btn btn-secondary moto-admin-trigger" onClick={() => setOpen(true)}>
        ...
        <div className="moto-admin-header">
          <h2>
            {view === "list"
              ...
              : editId ? "Edytuj motocykl" : "Dodaj motocykl"}
          </h2>
                ...
                {filtered.map((m) => (
                  <div className="moto-admin-item" key={m.id}>
                    ...
                    <div className="moto-admin-item-info">
                      ...
                      <span>{m.year} · {m.displacement} ccm · {m.power} KM · {m.price.toLocaleString("pl-PL")} zł</span>
                      <span className={`moto-admin-status moto-admin-status--${m.status}`}>{STATUS_LABELS[m.status]}</span>
                    </div>
                    <div className="moto-admin-item-actions">
                      <button className="btn btn-sm btn-secondary" onClick={() => openEdit(m)}>Edytuj</button>
                      {deleteConfirm === m.id ? (
                        <div style={{ display: "flex", gap: 6 }}>
                          <button className="btn btn-sm btn-danger" onClick={() => handleDelete(m.id)} disabled={saving}>Potwierdź</button>
                          <button className="btn btn-sm btn-secondary" onClick={() => setDeleteConfirm(null)}>Anuluj</button>
                        </div>
                      ) : (
                        <button className="btn btn-sm btn-danger" onClick={() => setDeleteConfirm(m.id)}>Usuń</button>
                      )}
                    </div>
                  </div>
                ))}
            ...
            <div className="moto-admin-form">
              <div className="moto-form-grid">
                <label className="moto-form-field">
                  <span>Marka *</span>
                  <input type="text" value={form.brand} onChange={(e) => setForm((p) => ({ ...p, brand: e.target.value }))} placeholder="np. Kawasaki" list="brand-suggestions" />
                  <datalist id="brand-suggestions">
                    {["Kawasaki", "Benelli", "Kymco", "Yamaha", "Honda", "Suzuki", "BMW", "KTM", "Ducati", "Harley-Davidson"].map((b) => <option key={b} value={b} />)}
                  </datalist>
                </label>
                <label className="moto-form-field">
                  <span>Model *</span>
                  <input type="text" value={form.model} onChange={(e) => setForm((p) => ({ ...p, model: e.target.value }))} placeholder="np. Z650" />
                </label>
                <label className="moto-form-field">
                  <span>Kategoria</span>
                ...
                <label className="moto-form-field moto-form-field--full">
                  <span>Opis</span>
                  <textarea value={form.description} onChange={(e) => setForm((p) => ({ ...p, description: e.target.value }))} rows={3} placeholder="Opis motocykla..." />
                </label>
              </div>
              ...
              <div className="moto-form-footer">
                ...
                <button className="btn btn-primary" onClick={handleSave} disabled={saving || !form.brand || !form.model}>
                  {saving ? <><span className="moto-spinner" /> Zapisywanie…</> : editId ? "Zapisz zmiany" : "Dodaj motocykl"}
                </button>
              </div>
            </div>
          )}
