...
> **Notka o zrzutach ekranu**
> W dokumencie znajdziesz markery typu `{{ ZRZUT: ... }}` — to miejsca, w których
> warto wstawić screenshot. Zrób je sam w przeglądarce (Ctrl+PrintScreen / Snipping
> Tool / Mac: ⌘+Shift+4), zapisz w `docs/zrzuty/` i podmień marker na obrazek:
> `![opis](zrzuty/01-logowanie.png)`.
...
{{ ZRZUT: ekran logowania (strona /login) z polami login/hasło }}
...
{{ ZRZUT: cały dolny pasek admina na dowolnej podstronie }}
...
{{ ZRZUT: dowolna podstrona w trybie edycji z widocznymi czerwonymi obwódkami }}
...
{{ ZRZUT: kafelek produktu z najechaną myszką pokazujący overlay "Zmień obrazek" }}
...
{{ ZRZUT: żółty panel admina nad quizem (widoczny po włączeniu Trybu edycji na /dla-dzieci) }}
...
{{ ZRZUT: button "Zarządzaj częściami" na /czesci-uzywane w trybie edycji }}
...
{{ ZRZUT: panel admina dla części z listą i przyciskami Edytuj/Usuń }}
...
{{ ZRZUT: rozwinięte menu Przywracanie z 3 opcjami }}
...
{{ ZRZUT: przycisk 🔑 Konto w toolbarze admina }}
...
{{ ZRZUT: modal "Zmiana danych logowania" z polami formularza }}
